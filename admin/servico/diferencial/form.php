...
$url_prev = $pathSite . "admin/{$tabelaPai}/diferencial/" . cr($fk) . "/lista";
//$objCategoria = new Base("servico_ctg");
//$categorias = fetch_all($objCategoria->consulta("", 'ORDER BY ordem, titulo'));
?>
<style>
    .third{
        width: 31.33%;
...
<form class="pnl-form table-responsive col-xs-12 pdg-0 pdg-t-20" method="POST" action="" enctype="multipart/form-data">
    ...
    <div class="col-xs-12 pdg-0 form-group">
        <label class="titulo">Texto</label>
        <textarea class="form-control tinymce" placeholder="Descreva..." rows="6" name="texto" ><?= $_POST["texto"] ?></textarea>
    </div>
    <div class="form-group">
        <label>Foto</label>
        <input type="file" class="form-control" name="nomeFoto"><br/><br/>
    </div>
