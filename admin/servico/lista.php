...
if ($_POST["ft_title"]) {
    ...
    if (!empty($_POST["ft_title"])) {
        $ft_aux .= " servico.titulo LIKE '%{$ft_title}%' ";
    }
}

$consulta = fetch_all($obj->consultaSelect("servico.id, "
        . "servico.titulo, "
        . "servico.nome_foto ",
        "$ft_aux",
        "ORDER BY servico.ordem ASC, servico.data_cadastro DESC, servico.id DESC"));
        ...
        <tbody id="sortable">
            ...
            if (!empty($consulta)) {
                foreach ($consulta as $campo) {
                    $qtd_top = $obj_top->totalRegistros("WHERE servico_fk=".$campo["id"]);
