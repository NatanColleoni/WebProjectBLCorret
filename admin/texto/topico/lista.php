    ...
    <table class="table">
        <thead>
            <tr class="text-center-f">
                ...
                <th width="87%" class="text-left">Título</th>
        ...
        <tbody id="sortable">
            ...
            if(!empty($consulta)) {
                foreach ($consulta as $campo) {
                    ...
                    $img = rightImg($campo["nome_foto"], $tabelaPai."/files/");
                    ?>
                    <tr id="targ-i-<?=$campo["id"]?>" class="sort bg-line-zebra text-center" rel="<?=$campo["id"]?>" height="30">
                        <td>
                            <? if(!in_array($fk, $nao_excluir)) { ?>
                                <input type="checkbox" class="chk-item" name="excluir[]" value="<?= $campo["id"] ?>" />
                            <? } ?>
                        </td>
                        <td class="text-left" ><a href="<?=$url_info?>"><?=$campo["titulo"]?></a></td>
                        <!--<td><a class="dsp-in-block" href="<?=$url_info?>"><img width="30" alt="<?=$campo["titulo"]?>" title="<?=$campo["titulo"]?>" src="<?=$img["nl"]?>" /></a></td>-->
                    </tr>
                    <?
                }
            }
