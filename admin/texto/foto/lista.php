...
if(!empty($fk)){
    $consulta = fetch_all($obj->consulta("WHERE {$tabelaPai}_fk='$fk'", "ORDER BY ordem ASC"));
...
$url_prev = $pathSite."admin/{$tabelaPai}/".cr($pai["page_fk"])."/".arruma_string($pai["titulo"]);
        ...
        <tbody id="sortable">
            ...
            if(!empty($consulta)){
                foreach($consulta as $campo){
                    ...
                    $img=rightImg($campo["nome_foto"],"texto/files/");
                    ?>
